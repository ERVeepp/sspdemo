<template lang="html">
  <div class="">
    ...
    <div class="Docker mt-40">
      <el-tabs type="card" class='thirdreport-tab'
      v-model='tabactive'
      >
        <el-tab-pane
        v-for='(item, index) in reportcontents.configure.thirdreport.taboptions'
        :key='item.value'
        :label="item.label"
...
export default {
  ...
  created () {
    this.tabactive = this.reportcontents.configure.thirdreport.taboptions[0].value
    this.setdatepicker()
  },
  mounted () {},
  methods: {
    ...
    searchdata () {
      if (this.date) {
        ...
        this.getdata(this.reportcontents.configure.thirdreport.taboptions.filter((ele) => {
      ...
      for (let i in this.reportdata) {
        if (i === data.value) {
          ...
          if (datelen) {
            ...
            this.showechart(data.label, 15, this.reportdata[i].slice((datalength - 15), datalength))
          }
        }
      }
    },
